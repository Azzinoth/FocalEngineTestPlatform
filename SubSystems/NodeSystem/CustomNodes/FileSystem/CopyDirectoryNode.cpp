#include "CopyDirectoryNode.h"
#include "../../../FEFileSystem.h"
using namespace VisNodeSys;

bool CopyDirectoryNode::bIsRegistered = []()
{
	auto Creator = []() -> Node* {
		return new CopyDirectoryNode();
	};

	auto Copier = [](const Node& CurrentNode) -> Node* {
		const CopyDirectoryNode& NodeToCopy = static_cast<const CopyDirectoryNode&>(CurrentNode);
		return new CopyDirectoryNode(NodeToCopy);
	};

	NODE_FACTORY.RegisterNodeType("CopyDirectoryNode", Creator, Copier);
	return true;
}();

CopyDirectoryNode::CopyDirectoryNode() : BaseExecutionFlowNode()
{
	Type = "CopyDirectoryNode";

	SetStyle(DEFAULT);
	SetName("Copy Directory");
...
CopyDirectoryNode::CopyDirectoryNode(const CopyDirectoryNode& Other) : BaseExecutionFlowNode(Other)
{
	SetStyle(DEFAULT);
	Result = Other.Result;
	Output[1]->SetFunctionToOutputData(ResultDataGetter);
...
Json::Value CopyDirectoryNode::ToJson()
{
	return Node::ToJson();
}

bool CopyDirectoryNode::FromJson(Json::Value Json)
{
...
void CopyDirectoryNode::SocketEvent(NodeSocket* OwnSocket, NodeSocket* ConnectedSocket, NODE_SOCKET_EVENT EventType)
{
	...
	if (EventType == EXECUTE)
	{
		...
		Result = FocalEngine::FILE_SYSTEM.CopyDirectory(SourceInput, DestinationInput);
...
bool CopyDirectoryNode::CanConnect(NodeSocket* OwnSocket, NodeSocket* CandidateSocket, char** MsgToUser)
{
