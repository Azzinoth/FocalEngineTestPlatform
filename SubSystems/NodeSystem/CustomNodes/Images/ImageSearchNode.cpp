...
bool ImageSearchNode::bIsRegistered = []()
{
	...
	NODE_FACTORY.RegisterNodeType("ImageSearchNode", Creator, Copier);
	return true;
}();
...
ImageSearchNode::ImageSearchNode() : BaseExecutionFlowNode()
{
	...
	AddSocket(new NodeSocket(this, "IMAGE", "Match", NodeSocket::SocketFlow::Output));
	Output[4]->SetFunctionToOutputData(ImageDataGetter);
}
...
bool ImageSearchNode::FromJson(Json::Value Json)
{
	...
	// Here I am restoring the output data function.
	// Because the function is not serializable, I have to set it manually.
	if (Output.size() < 5 || Output[0] == nullptr || Output[1] == nullptr || Output[2] == nullptr || Output[3] == nullptr || Output[4] == nullptr)
		return false;
